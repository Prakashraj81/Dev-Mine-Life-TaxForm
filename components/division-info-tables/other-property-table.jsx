...
export default function OthersPropertyTable({ heir_details_list }) {
  ...
  let [ApiCallRoute, setApiCallRoute] = useState("other_assets");
  ...
  let [OthersPropertyList, setOthersPropertyList] = useState([]);
  let [HeirSharingDetails, setHeirSharingDetails] = useState([]);
  let [SnackbarOpen, setSnackbarOpen] = useState(false);
  let [SnackbarMsg, setSnackbarMsg] = useState("Others property split details saved successfully.");

  useEffect(() => {
    GetOthersPropertyList();
  ...
  const GetHeirSharingDetails = async (Id) => {
    ...
    if (auth_key !== null && Id !== 0) {
      try {
        const response = await fetch(`https://minelife-api.azurewebsites.net/get_other_assets_details?auth_key=${auth_key}&id=${Id}`);
  ...
  const GetOthersPropertyList = async () => {
    const auth_key = atob(localStorage.getItem("mine_life_auth_key"));
    if (auth_key !== null) {
      try {
        ...
        if (response.ok) {
          TotalAmount = 0;
          setOthersPropertyList(data.other_assets_details);
          {data.other_assets_details.map((list) => {
              if (list.valuation !== 0) {
                TotalAmount = TotalAmount + list.valuation;
      ...
      <div className="py-0">
        <Table aria-label="collapsible table">
          <TableHead className="table-head">
            <TableRow>
              <TableCell className="" align="left" style={{border: 'none'}}><span className="font-medium">その他財産</span></TableCell>
              <TableCell className="invisible" align="left" style={{border: 'none'}}><span className="font-medium">Column</span></TableCell>
              <TableCell className="invisible" align="left" style={{border: 'none'}}><span className="font-medium">Column</span></TableCell>
              <TableCell className="table-20" align="right" style={{border: 'none'}}>{TotalAmount.toLocaleString()}<span className="inline-block float-right border-l text-right pl-1">円</span></TableCell>
              <TableCell className="cursor-pointer" align="right" style={{border: 'none'}}>
                  ...
                  <Box className="my-2">
                    <Table>
                      <TableHead className="table-head-2">
                        ...
                        {OthersPropertyList.map((list) => (
                          <React.Fragment key={list.id}>
                            <TableRow key={list.id} id={list.id} value={list.customer_id}>
                              <TableCell className="border border-light-gray border-l">{list.property_name}</TableCell>
                              <TableCell className="border border-light-gray border-l">{list.other_party}</TableCell>
                              ...
                              <TableCell className="border border-light-gray border-l w-15" align="center">
                                <IconButton
                                  ...
                                  value={list.valuation.toLocaleString()}
