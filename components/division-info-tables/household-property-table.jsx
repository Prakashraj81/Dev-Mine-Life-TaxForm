...
export default function HouseholdPropertyTable({ heir_details_list }) {
  ...
  let [ApiCallRoute, setApiCallRoute] = useState("household");
  ...
  let [HouseholdList, setHouseholdList] = useState([]);
  let [HeirSharingDetails, setHeirSharingDetails] = useState([]);
  let [SnackbarOpen, setSnackbarOpen] = useState(false);
  let [SnackbarMsg, setSnackbarMsg] = useState("Household split details saved successfully.");
  ...
  const GetHeirSharingDetails = async (Id) => {
    ...
    if (auth_key !== null && Id !== 0) {
      try {
        const response = await fetch(`https://minelife-api.azurewebsites.net/get_household_details?auth_key=${auth_key}&id=${Id}`);
  ...
  const GetHouseholdList = async () => {
    ...
    if (auth_key !== null) {
      try {
        ...
        if (response.ok) {
          TotalAmount = 0;
          setHouseholdList(data.household_details);
      ...
      <div className="py-0">
        <Table aria-label="collapsible table">
          <TableHead className="table-head">
            <TableRow>
              <TableCell className="" align="left" sx={{border: 'none'}}><span className="font-medium">家庭用財産</span></TableCell>
...
HouseholdPropertyTable.propTypes = {
